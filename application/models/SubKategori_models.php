...
class SubKategori_models extends CI_Model {
    ...
    {
        return $this->db->query("SELECT d.kode_class, c.kode_subclass, b.kode_kategori, a.kode_subkategori, a.nama_subkategori,
    ...
    public function subcode($kode_kategori)
    {
      $this->db->select('RIGHT(tk_subkategori.kode_subkategori,3) as kode_subkategori', FALSE);
      ...
      $kodetampil =  $kode_kategori . $batas;  //format kode
      // var_dump($kodetampil); die();
      return $kodetampil;
    }


    public function subcodeedit($kode_kategori)
    {
      $this->db->select('RIGHT(tk_subkategori.kode_subkategori,3) as kode_subkategori', FALSE);
      $this->db->where('kode_kategori', $kode_kategori);
      // $this->db->where('kode_warna', $kode_warna);
      ...
      if ($query->num_rows() <> 0) {
        ...
        // $kode = intval($data->kode_class) + 1;
        //cara mengambil digit dari kode kode sub class dari belakang
        $kode = substr($data->kode_subkategori, -3);
      } else {
        $kode = 1;  //cek jika kode belum terdapat pada table
      }
      // $tgl = date('ym');
